...
// NOTE CHECK IF THIS IS THE FIRST TIME THE USER HAS LOGGED IN
$newby_results = mysqli_query($link, "SELECT * FROM challenge_status WHERE badge_id = 66 AND user_id = $user_id");

if (mysqli_num_rows($newby_results) == 0){
  mysqli_query($link, "INSERT INTO challenge_status (user_id, challenge_id, badge_id) VALUES ($user_id, 70, 66)");
}
...
// <----- SECTION CODE FUNCTION: GET BADGES AND DISPLAY THEM ----->
    ...
    // NOTE IF USER HAS THIS BADGE
    $b_img_result = mysqli_query($link, "SELECT cd.name, cd.description, badge_id, img_path FROM badge, challenge_data AS cd WHERE cd.challenge_id = badge.challenge_id AND badge_id IN (SELECT badge_id FROM challenge_status WHERE user_id = $user_id AND status = 'COMPLETED')");
    ...
    $b_img_result = mysqli_query($link, "SELECT cd.name, cd.description, badge_id, img_path FROM badge, challenge_data AS cd WHERE cd.challenge_id = badge.challenge_id AND badge_id IN (SELECT badge_id FROM challenge_status WHERE user_id = $user_id AND status = 'PENDING')");
    ...
    while($row = mysqli_fetch_assoc($b_img_result)) {
      ...
      $f_challenges[] = $row;
        ...
        // NOTE IF THE USER DOES NOT HAVE THIS BADGE
...
usort($t_challenges, cmp);
usort($f_challenges, cmp);
$result = array_merge($t_challenges, $f_challenges);

        // NOTE ECHO ALL BADGES AND THEIR LIGHTBOX
          foreach ($result as $all_ch) {
